...
static uint32_t BlockHash_1_Q[4096],BlockHash_1_c,BlockHash_1_i;
uint8_t *BlockHash_1_MemoryPAD8;
uint32_t *BlockHash_1_MemoryPAD32_to_8;
uint32_t *BlockHash_1_MemoryPAD32;

uint32_t BlockHash_1_rand(void)
{
	uint32_t& pos = BlockHash_1_Q[BlockHash_1_i&0xFFF];
    uint64_t t = 0x495ELL * pos + BlockHash_1_c;
    BlockHash_1_c = (t >> 32);
    uint32_t x = (t + BlockHash_1_c)&0xFFFFFFFF;
    if (x < BlockHash_1_c)
    {
        x++;
        BlockHash_1_c++;
...
void BlockHash_Init()
{
	...
	{ 	
		...
		BlockHash_1_MemoryPAD32 = new uint32_t [BLOCKHASH_1_PADSIZE];
		for(uint32_t i=0; i<BLOCKHASH_1_PADSIZE; ++i)
			BlockHash_1_MemoryPAD32[i] = *(uint32_t*)(BlockHash_1_MemoryPAD8+i);
	} 	
...
const uint32_t PAD_MASK = BLOCKHASH_1_PADSIZE-1;
...
#define WORKINIT(a,b,c)   work3[a] ^= work2[c]; \
	...
	work3[0] = work2[15];
    for(int32_t x=1;x<320;x++)
    {
		WORKINIT(x-1, x, x&63);
    }
	
	uint64_t qCount = *((uint64_t*)&work3[310]);
    int32_t nExtra=(READ_PAD8(qCount+work3[300])>>3)+512;
    for(int32_t x=1;x<nExtra;x++)
    {
...
#else
//256
uint32_t remu320(uint64_t x) 
{
	uint32_t n = (((x>>38UL) + ((x>>6)&0xFFFFFFFF)) >> 32) + ((x>>38UL) + ((x>>6)&0xFFFFFFFF)&0xFFFFFFFF);
	return ((n%5)<<6)+(x&63U);
}
//96
uint32_t remu160(uint64_t x) 
{
	uint32_t n = (((x>>37UL) + ((x>>5)&0xFFFFFFFF)) >> 32) + ((x>>37UL) + ((x>>5)&0xFFFFFFFF)&0xFFFFFFFF);
	return ((n%5)<<5)+(x&31U);
}
//208
uint32_t remu316(uint64_t x)
{
	x = (x >> 32)*208 + (uint32_t)x;
	x = (x >> 32)*208 + (uint64_t)x;
	return ((uint64_t)x + (uint64_t)(x>>32)*208U)%316;
}
#endif
...
void BlockHash_1_mine_V3(uint8_t *p512bytes, uint8_t* final_hash, uint8_t* results)
{
    ...
    uint8_t* work1 = p512bytes;
	uint8_t* xork1 = p512bytes+512;
	uint8_t* york1 = p512bytes+1024;
    uint8_t* work2=work1+128;
	uint8_t* xork2=xork1+128;
	uint8_t* york2=york1+128;
    uint8_t* work3=work1+192;
	uint8_t* xork3=xork1+192;
	uint8_t* york3=york1+192;
...
#define sCount counts[2]
	qCount = *((uint64_t*)&work3[310]);
	rCount = *((uint64_t*)&xork3[310]);
	sCount = *((uint64_t*)&york3[310]);
    int32_t nExtra=(READ_PAD8(qCount+work3[300])>>3)+512;
    int32_t oExtra=(READ_PAD8(rCount+xork3[300])>>3)+512;
    int32_t pExtra=(READ_PAD8(sCount+york3[300])>>3)+512;
	int32_t x,y,z;
    for(x=1,y=1,z=1;x<nExtra && y<oExtra && z<pExtra;x++,y++,z++)
    {
        ...
        qCount+= READ_PAD32( *((uint32_t*)&work3[qCount&0xFF]) );
        rCount+= READ_PAD32( *((uint32_t*)&xork3[rCount&0xFF]) );
        sCount+= READ_PAD32( *((uint32_t*)&york3[sCount&0xFF]) );

		work3[x%320]=work2[x&63]^uint8_t(qCount);
		xork3[y%320]=xork2[y&63]^uint8_t(rCount);
		york3[z%320]=york2[z&63]^uint8_t(sCount);

        qCount+= READ_PAD32( (qCount>>32)+work3[x%200] );
        rCount+= READ_PAD32( (rCount>>32)+xork3[y%200] );
        sCount+= READ_PAD32( (sCount>>32)+york3[z%200] );

		*((uint32_t*)&work3[remu316(qCount)]) ^= (qCount>>24)&0xFFFFFFFF;
        *((uint32_t*)&xork3[remu316(rCount)]) ^= (rCount>>24)&0xFFFFFFFF;
        *((uint32_t*)&york3[remu316(sCount)]) ^= (sCount>>24)&0xFFFFFFFF;
